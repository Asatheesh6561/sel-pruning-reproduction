...
def get_top_frac(values_tensor: Tensor, top_frac: float) -> Tuple[Tensor, float]:
    ...
    topk_values = torch.topk(
        values_tensor.flatten(), k, dim=-1, largest=True, sorted=False
    )
...
def get_activation_data(
    m: Model, dataset_name: str, split: str = "train", num_tokens: int = 1e5
) -> Dict[str, Tensor]:
    ...
    for data in tqdm(ds):
        ...
        attn_act = einops.rearrange(
            attn_act, "batch layer token head pos -> (batch token) layer head pos"
        )

        mlp_act = m.collect_recent_mlp_pre_out()
        mlp_act = einops.rearrange(
            mlp_act, "batch layer token pos -> (batch token) layer pos"
        )
...
def run_selective_pruning(
    ...
    print(
        f"Running selective pruning with mlp_frac = {mlp_frac}, attn_frac = {attn_frac}"
    )
...
if __name__ == "__main__":
    ...
    m = Model("meta-llama/Meta-Llama-3-8B", limit=1000, dtype="fp32")

    # Define parameters for the selective pruning
    retain_dataset = "wikitext"
    forget_dataset = "bio-remove"
    all_datasets = [retain_dataset, forget_dataset]
    eval_sample_size = 10000  # bigger tends to be better
    collect_sample_size = 10000  #
    ...
    results_before = evaluate_all(
        m,
        sample_size=eval_sample_size,
        datasets=all_datasets,
        dataset_tokens_to_skip=collect_sample_size,
    )
    ...
    run_selective_pruning(
        m,
        retain_dataset,
        forget_dataset,
        mlp_frac=mlp_frac,
        attn_frac=attn_frac,
        sample_size=collect_sample_size,
    )

    print("## Running evaluation after pruning")
    results_after = evaluate_all(
        m,
        sample_size=eval_sample_size,
        datasets=all_datasets,
        dataset_tokens_to_skip=collect_sample_size,
    ...
    print(
        "pile loss before:",
        results_before["loss_data"]["wikitext"]["loss"],
        "-> after:",
        results_after["loss_data"]["wikitext"]["loss"],
    )
    print(
        "code loss before:",
        results_before["loss_data"]["bio-remove"]["loss"],
        "-> after:",
        results_after["loss_data"]["bio-remove"]["loss"],
    )
    print(
        f'pile top1 accuracy before: {results_before["accuracy"]["wikitext"]["base"]:.2f}%',
        f'-> after: { results_after["accuracy"]["wikitext"]["base"]:.2f}%',
    )
    print(
        f'code top1 accuracy before: {results_before["accuracy"]["bio-remove"]["base"]:.2f}%',
        f'-> after: { results_after["accuracy"]["bio-remove"]["base"]:.2f}%',
    )
